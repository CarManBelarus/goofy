...
8. Окно закроется. Выберите `Начать развертывание` - `Пробные развертывания`
   ...
   > Если будет ошибка `Не удалось открыть файл` или другие про недоступность страницы, перейдите по ссылке в режиме инкогнито или другом браузере. Из-за авторизации Google в нескольких аккаунтах в текущем браузере, происходит ошибочное перенаправление на другой аккаунт где страница недоступна.
